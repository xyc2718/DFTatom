...
def create_minimal_hydrogen_basis() -> BasisSet:
    """
    创建氢原子的最小基组（仅1s轨道）
    用于测试目的
    """
    ...
    r_max = 10.0
    n_points = 1000
    r_grid = np.linspace(0.001, r_max, n_points)  # 避免r=0
    
    # 氢原子1s轨道的解析形式: R_10(r) = 2 * exp(-r)
    radial_1s = 2.0 * np.exp(-r_grid)
    
    # 创建基组
    basis_set = BasisSet('H')
    
    # 添加1s轨道
    orbital_1s = AtomicOrbital(
        n=1,
        l=0,
        m=0,
        orbital_type=0,
        n_index=0,
        r_grid=r_grid,
        radial_function=radial_1s
    )
    
    basis_set.add_orbital(orbital_1s)
    basis_set.set_basis_parameters(energy_cutoff=100.0, radius_cutoff=10.0)
...
def example_usage():
    ...
    # 方法1: 从文件加载
    try:
        carbon_basis = load_basis_set_from_file('C_gga_7au_100Ry_2s2p1d.orb')
        carbon_basis.print_basis_info()
        
        # 访问特定轨道
        orbital_1s = carbon_basis.get_orbital_by_quantum_numbers(1, 0, 0)  # 1s轨道
        if orbital_1s:
            print(f"\n1s轨道名称: {orbital_1s.orbital_name}")
            print(f"1s轨道在r=1.0处的值: {orbital_1s.evaluate_radial(1.0)}")
        
        # 获取所有p轨道
        p_orbitals = carbon_basis.get_orbitals_by_l(1)
        print(f"\np轨道数量: {len(p_orbitals)}")
        for orbital in p_orbitals:
            print(f"  {orbital.orbital_name}")
            
    except FileNotFoundError:
        print("碳原子基组文件未找到，使用氢原子测试基组")
        
        # 方法2: 使用测试基组
        hydrogen_basis = create_minimal_hydrogen_basis()
        hydrogen_basis.print_basis_info()
